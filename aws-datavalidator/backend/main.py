...
def call_customer_api(payload: Dict) -> Dict:
    ...
    if "hdr" in payload_to_send:
        ...
        # Keep pay_amt as string (downstream .NET API expects string, not number)
        if "pay_amt" in hdr:
            try:
                hdr["pay_amt"] = str(float(hdr["pay_amt"])) if hdr["pay_amt"] else "0.0"
    ...
    # Send directly — NO wrapper (matches Postman format that works)
